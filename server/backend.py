...
def send_to_game(gyroX, gyroZ, trigger, ability, calibration):
    input_data = struct.pack('<ff???', gyroX, gyroZ, trigger, ability, calibration)
...
receive_thread = threading.Thread(target=receive_from_game)
send_thread = threading.Thread(target=receive_from_controller)
